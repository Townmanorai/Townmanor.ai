...
const PropertyLocation = ({ lat, lng }) => { // Accept lat and lng as props
  const [routeFrom, setRouteFrom] = useState("");
  ...
  return (
    <div className="map-dv widget-listing-map">
      ...
      <div className="fullwidth-home-map">
        {lat && lng ? ( // Check if lat and lng are provided
          <>
            <div className="property-map" id="property-map" style={{ height: '385px' }}>
              <Map results={[{ lat, lng }]} /> {/* Pass lat and lng to the Map component */}
            </div>
          </>
        ) : (
