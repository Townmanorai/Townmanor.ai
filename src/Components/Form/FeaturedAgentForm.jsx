...
const FeaturedAgentForm = () => {
  ...
  useEffect(() => {
    if (selectedListing === 'self') {
      axios.get(`https://www.townmanor.ai/api/user/${username}`)
        .then((response) => {
          setFormData({
            ...formData,
  ...
  useEffect(() => {
    axios.get(`https://www.townmanor.ai/api/userplanslimit/${username}`)
      .then((response) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (parseInt(agentLimits[agentLimitField]) > 0) {    
      try {
          ...
          if (selectedAgentType === 'spotlight') {
            const propertyId = formData.propertyId;
            const propertyResponse = await axios.get(`https://www.townmanor.ai/api/property/${propertyId}`);
        ...
        // Add the agent to the ad agents table
        const agentResponse = await axios.post('https://www.townmanor.ai/api/adsagents', { 
          ...formData, 
          agentType: selectedAgentType 
        });
        console.log("agentResponse", agentResponse);
        ...
        console.log("updatedLimits", updatedLimits);
        await axios.put(`https://www.townmanor.ai/api/userplanslimit/${username}`, updatedLimits);
        ...
        await axios.put(`https://www.townmanor.ai/api/property/agents/${formData.propertyId}`, {
